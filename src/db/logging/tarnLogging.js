...
module.exports.initializeTarnLogging = (pool, { logger = console } = {}) => {
  ...
  pool.on('acquireSuccess', (eventId, resource) => logger.debug(`acquireSuccess: ${message(eventId, resource)}`.yellow))
  ...
  pool.on('destroyRequest', (eventId, resource) => logger.debug(`destroyRequest: ${message(eventId, resource)}`.yellow))
  pool.on('destroySuccess', (eventId, resource) => logger.debug(`destroySuccess: ${message(eventId, resource)}`.yellow))
